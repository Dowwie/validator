...
pub(crate) fn validate_single_label(
    ...
    for episode in dataset.episodes {
        ...
        let label = match episode.expected {
            wire::Target::Class { label } => vocabulary
                .lookup(&label)
        ...
        if expected
            .insert(id, Episode::new(id, episode.input, label))
            .is_some()
        {
    ...
    if predictions.len() != selected_ids.len()
        || predictions.keys().any(|id| !selected_ids.contains(id))
    {
...
pub(crate) fn validate_multi_label(
    ...
    for episode in dataset.episodes {
        ...
        let target = match episode.expected {
            wire::Target::Labels { labels } => {
                vocabulary.label_set(labels.iter().map(String::as_str))?
            }
            wire::Target::Class { .. } => return Err(Diagnostic::for_code(DiagnosticCode::Config)),
        };
        if expected
            .insert(id, Episode::new(id, episode.input, target))
            .is_some()
        {
            return Err(Diagnostic::for_code(DiagnosticCode::Id));
        }
...
mod tests {
    ...
    fn multi_label_checked_admission() {
        ...
        duplicate_row["predictions"]
            ...
            .unwrap()
            .push(first_row);
        assert_multi_code(&dataset, &duplicate_row, &config, DiagnosticCode::Id);
    ...
    fn opaque_number_and_null() {
        ...
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(episode.input.get()).unwrap()["number"]
                .as_u64(),
    ...
    #[test]
    fn malformed_artifact_fails_before_population_selection() {
        let digest = crate::model::common::ArtifactDigest::try_from(
            "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
        )
        .unwrap();
    ...
    fn json_number_marker_collision() {
        ...
        assert_eq!(
            golden.value.episodes[0].input.get(),
