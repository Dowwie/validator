...
mod tests {
    ...
    #[test]
    fn metric_status_precedence() {
        for (expected, predicted, status) in [
            (vec![], vec![], MetricStatus::NoData),
            (
                vec![&["A"][..]],
                vec![None],
                MetricStatus::NoAnsweredPredictions,
            ...
            let results = evaluated(&["A"], &expected, &predicted);
            assert_eq!(results.raw.answered_micro_f1.status_value(), status);
            assert_eq!(results.raw.answered_micro_f1.value(), None);
            assert_eq!(results.raw.answered_micro_f1.numerator(), Some(0));
            assert_eq!(results.raw.answered_micro_f1.denominator(), Some(0));
            assert_eq!(
                results.probability.mean_binary_brier.status_value(),
                MetricStatus::NotApplicable
            );
        }
    }
    ...
    #[test]
    fn macro_undefined_classes() {
        let results = evaluated(&["A", "B"], &[&["A"]], &[Some(&["A"])]);
        let metric = &results.raw.answered_macro_f1;
        assert_eq!(
            metric.metric.status_value(),
            MetricStatus::ContainsUndefinedClasses
        );
        assert_eq!(metric.metric.value(), Some(0.5));
        assert_eq!(metric.undefined_classes, ["B"]);
        for (expected, predicted, status) in [
            (vec![], vec![], MetricStatus::NoData),
            (
                vec![&["A"][..]],
            ...
            let results = evaluated(&["A", "B"], &expected, &predicted);
            assert_eq!(results.raw.answered_macro_f1.metric.status_value(), status);
            assert_eq!(results.raw.answered_macro_f1.metric.value(), None);
            assert!(results.raw.answered_macro_f1.undefined_classes.is_empty());
        }
    }
