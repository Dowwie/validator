...
mod admission_tests {
    ...
    fn validate_before_selection() {
        let invalid_id = EpisodeId::try_from("not-an-id");
...
/// An episode that owns its opaque input once and its concrete expected target.
#[derive(Debug)]
...
impl<Target> Episode<Target> {
    ...
    pub fn new(id: EpisodeId, input: Box<RawValue>, target: Target) -> Self {
    ...
    /// Borrows the opaque input without copying it.
    pub fn input(&self) -> &RawValue {
        &self.input
    }
...
impl EvaluationConfig<crate::model::single_label::SingleLabelPolicy> {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            policy: crate::model::single_label::SingleLabelPolicy::AsRecorded,
        })
    }
}
...
impl<Policy> Population<Policy> {
    ...
    /// Restricts this checked population to already selected episode IDs.
    pub(crate) fn restrict(self, mut selected: Vec<EpisodeId>) -> Result<Self> {
        selected.sort();
        if selected.windows(2).any(|pair| pair[0] == pair[1])
            || selected
                .iter()
                .any(|id| self.selected.binary_search(id).is_err())
        {
            return Err(Diagnostic::for_code(DiagnosticCode::Id));
        }
        ...
        unselected.extend(
            self.selected
                .into_iter()
                .filter(|id| selected.binary_search(id).is_err()),
        );
        ...
        Self::new(
            ...
            EvaluationConfig {
                ...
                role,
                parent_run_id,
                requested_episode_ids: Some(selected.clone()),
                policy,
            },
            selected,
            unselected,
...
impl MetricResult {
    /// Constructs a finite ratio metric for unit tests.
    #[cfg(test)]
    pub fn ratio(
        numerator: u64,
        denominator: u64,
        ...
        if denominator == 0 {
            return Self::ratio_with_population(numerator, denominator, 0, scope, unit);
        }
        Self::ratio_with_population(numerator, denominator, denominator, scope, unit)
    }
...
mod tests {
    use super::{
        ArtifactDigest, Episode, EpisodeId, LabelVocabulary, Outcome, RawValue, RunId, SourceId,
    };
    ...
    fn model_boundary_visibility() {
        ...
        assert_eq!(episode.input().get(), "null");
