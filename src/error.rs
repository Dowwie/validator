...
impl Diagnostic {
    ...
    ///
    /// Callers attach filesystem paths and checked episode identities separately;
    /// opaque payload text does not belong in a diagnostic.
    #[must_use]
    ...
    pub(crate) fn with_affected_ids(
        mut self,
        ids: impl IntoIterator<Item = crate::model::common::EpisodeId>,
    ) -> Self {
        let mut ids = ids.into_iter().collect::<Vec<_>>();
        ids.sort();
        ids.dedup();
        self.affected_ids = ids.iter().map(ToString::to_string).collect();
        self
    }
    ...
    /// Returns the prescribed exit category without examining the message.
    #[must_use]
    pub const fn exit_category(&self) -> ExitCategory {
