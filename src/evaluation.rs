...
mod tests {
    use super::*;
    use crate::model::common::{MetricResult, MetricScope, MetricStatus, MetricUnit};

    #[test]
    fn metric_status_precedence() {
        assert!(MetricResult::ratio(0, 0, MetricScope::Selected, MetricUnit::Episode).is_ok());
        assert!(matches!(
            MetricResult::status(
    ...
    fn macro_undefined_classes() {
        let result = MetricResult::status(
            MetricStatus::NoData,
            0,
            MetricScope::Selected,
            MetricUnit::Episode,
